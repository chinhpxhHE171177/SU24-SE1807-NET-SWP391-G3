...
public class ApproveExamServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ApproveExamServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ApproveExamServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (txtSearch != null && !txtSearch.isEmpty()) {
            ...
            if (listq == null || listq.isEmpty()) {
                request.setAttribute("mess", "Not Found Exam");
            }
        } else {
            listq = qdao.getAllListQuiz();
        }
        ...
        request.getRequestDispatcher("adminExam.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (subjectId == 0 && statusValue == -1) {
            ...
            listq = qdao.getExamByStatus(statusValue);
        } else if (statusValue == -1) {
            listq = qdao.getExamByCoursetId(subjectId);
        } else {
            listq = qdao.getExamBySubjectAndStatus(subjectId, statusValue);
        }
