...
<%
    ...
    for (Permission p : permissions) {
        if ("DeviceManagement".equals(p.getPageName()) && p.isCanAccess()) {
            hasAccess = true;
            break;
        }
    }
...
<% 
   boolean canAdd = permissions.stream()
       .anyMatch(p -> "DeviceManagement".equals(p.getPageName()) && p.isCanAdd());
%>
                ...
                <td>
                   <% 
                       boolean canEdit = permissions.stream()
                           .anyMatch(p -> "DeviceManagement".equals(p.getPageName()) && p.isCanEdit());
                       boolean canDelete = permissions.stream()
                           .anyMatch(p -> "DeviceManagement".equals(p.getPageName()) && p.isCanDelete());
                   %>
                   <!-- Edit Button -->
                   <button class="btn" onclick="showEditModelForm('${d.id}', '${d.code}', '${d.name}', '${d.dateUse}', '${d.origin}', '${d.yom}', '${d.stageId}', '${d.issue}')"
                           <%= canEdit ? "" : "disabled" %>>
                       <i class="fas fa-pen"></i>
                   </button>

                   <!-- Delete Button -->
                   <button onclick="doDeleteDevice('${d.id}')" <%= canDelete ? "" : "disabled" %>>
                       <i class="fas fa-trash"></i>
                   </button>

                   <!-- View Detail Button (No permission required for viewing) -->
                   <button onclick="viewDetailModalForm('${d.code}', '${d.qrCode}', '${d.issue}')">
