            ...
            <form class="bg-white shadow-lg rounded-lg p-8 max-w-4xl w-full" action="quiz-manage" method="post" enctype="multipart/form-data">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start">
                    <div class="flex flex-col items-center text-center md:text-left md:items-start">
                        ...
                        <c:choose>
                            <c:when test="${QUIZ.image != null}">
                                <img src="data:image/png;base64,${QUIZ.image}" alt="Profile picture" id="profile-picture" class=" border-3 border-green-500 p-1 mb-3" style="width: 400px; cursor: pointer; margin: 10px auto;border: 2px solid #1b730d">
                            </c:when>
                            <c:otherwise>
                    ...
                    <div class="mt-8 md:mt-0 md:ml-10 w-full max-w-lg">
                        ...
                        <div class="space-y-4" >
                            <input type="file" id="image-input" name="image" style="display: none;" value="${QUIZ.image}">
                            <input type="hidden" name="leagueId" />
                            <div>
                                <label for="surname" class="text-gray-700">Quiz title *</label>
                                <input name="title" type="text" id="surname" value="${QUIZ.title}" class="w-full mt-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" required>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select id="level" name="level" class="w-full mt-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" required>
                                    ...
                                    <c:forEach var="value" begin="1" end="3">
                                        <option  ${value == QUIZ.level ? 'selected' : ''} value="${value}">${value}</option>     
                                    </c:forEach> 
                            ...
                            <div>
                                ...
                                <select name="subjectId" class="w-full mt-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"  requried>
                                    <c:forEach items="${SUBJECT}" var="subject">
                                        <option ${subject.id == QUIZ.subjectID ? 'selected' : ''} value="${subject.id}">${subject.name}</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                ...
                <div style="display: flex; justify-content: center; margin-top: 15px">
                    ...
                    <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded shadow peer-checked:bg-green-500 transition-colors" 
                            style="background-image: linear-gradient(to right top,#45af2a,#3ba023,#30901c,#268215,#1b730d,#1b730d,#1b730d,#1b730d,#268215,#30901c,#3ba023,#45af2a);">Update Quiz</button>
                </div>
            </form>
        </div>
    </body>
